...
export default function RequesterAccountSettingsScreen() {
  ...
  const handleDeleteAccount = () => {
    console.log('[Requester Account Settings] Delete Account pressed — opening confirmation modal');
    setShowDeleteModal(true);
  };

  const handleConfirmDelete = async () => {
    console.log('[Requester Account Settings] Delete Account confirmed, proceeding with deletion');
    setDeleting(true);
    try {
      // Force a fresh token before deletion — prevents 401 from expired sessions
      await supabase.auth.refreshSession();
      console.log('[Requester Account Settings] Calling delete-account edge function');
      ...
      console.log('[Requester Account Settings] delete-account response status:', res.status);
      if (!res.ok) {
        throw new Error('Account deletion failed on server. Please try again.');
      }
      console.log('[Requester Account Settings] Signing out after delete');
      await supabase.auth.signOut();
      router.replace('/');
    } catch (err: unknown) {
      console.error('[Requester Account Settings] Delete account error:', err);
      setDeleting(false);
    ...
    <>
      <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
        <TouchableOpacity
          ...
          onPress={() => { console.log('[Requester Account Settings] Back pressed'); router.back(); }}
          activeOpacity={0.7}
