...
export default function DoctorAccountSettingsScreen() {
  ...
  const handlePushToggle = async (value: boolean) => {
    console.log('[Doctor Account Settings] Push notifications toggle:', value);
    if (value) {
      if (permissionDenied) {
        // Previously denied — must go to OS Settings
        console.log('[Doctor Account Settings] Push permission denied, opening OS Settings');
        if (Platform.OS === 'ios') {
        ...
        console.log('[Doctor Account Settings] Requesting push permission');
        await requestPermission();
      }
    } else {
      // Cannot programmatically revoke — send to OS Settings
      console.log('[Doctor Account Settings] Cannot revoke push permission programmatically, opening OS Settings');
      if (Platform.OS === 'ios') {
        Linking.openURL('app-settings:');
      } else {
        Linking.openSettings();
  ...
  const handleLocationToggle = async (value: boolean) => {
    console.log('[Doctor Account Settings] Location toggle:', value);
    if (value) {
      if (!locationCanAskAgain) {
        // Permanently denied — must go to OS Settings
        console.log('[Doctor Account Settings] Location permanently denied, opening OS Settings');
        Linking.openSettings();
      } else {
        console.log('[Doctor Account Settings] Requesting location permission');
      ...
      // Cannot programmatically revoke — send to OS Settings
      console.log('[Doctor Account Settings] Cannot revoke location permission programmatically, opening OS Settings');
      Linking.openSettings();
    }
  };
    ...
    <>
      <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
        <TouchableOpacity
          ...
          onPress={() => {
            console.log('[Doctor Account Settings] Back pressed');
            router.back();
          }}
          activeOpacity={0.7}
      ...
      >
        ...
        <Card>
          <TouchableOpacity
            ...
            onPress={() => {
              console.log('[Doctor Account Settings] Navigate to Personal Data');
              router.push('/(doctor)/(account)/personal-data' as any);
            }}
            activeOpacity={0.7}
          >
